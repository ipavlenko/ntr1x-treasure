...
public class GoodResourceTest {
    ...
    public void test() {
        ...
        profiler.withDisabledSecurity(() -> {
            
            {
                IUserService.CreateUser s = new IUserService.CreateUser(); {
                    ...
                    s.name = "test";
                    s.middlename = "test";
                    s.confirmed = true;s.grants = new IGrantService.CreateGrant[] {
                        new IGrantService.CreateGrant("/goods", "admin")
                    };
                }
        ...
        profiler.withCredentials(target, "user0@example.com", "user0", (token) -> {
            ...
            Good good = null; {
                ...
                IGoodService.CreateRequest s = new IGoodService.CreateRequest(); {
                    ...
                    s.title = "First Good";
                    s.promo = "First Good Promo";
                    s.purchase = purchase.getId();
                    
                    IModificationService.RelatedModification m0 = new IModificationService.RelatedModification(); {
                        m0.price = 100.5f;
                    }
                    
                    s.modifications = new IModificationService.RelatedModification[] { m0 };
                }
                ...
                Assert.assertNotNull(good.getId());
                Assert.assertEquals(s.title, good.getTitle());
                Assert.assertNull(good.getModifications());
        ...
        profiler.withDisabledSecurity(() -> {
            ...
            {
                Good r = target
                    .path(String.format("/goods/i/%d", goods[0].getId()))
                    .request()
                    .delete(Good.class)
                ;
                
                Assert.assertEquals(goods[0].getId(), r.getId());
            }
